...
def mask_from_lens(lens, max_len=None):
    ...
    # lens: [B]
    mask = torch.lt(ids, lens.unsqueeze(1))  # [B, N]
    return mask
...
class PositionwiseConvFF(nn.Module):
    def __init__(self, d_model, d_inner, kernel_size, dropout, pre_lnorm=False):
        ...
        # 这里是 feed forward 的另一种写法，即用一维卷积实现，hidden size 在卷积中对应的就是 channel 的数量；这里如果 kernel size = 1 是完全等价的；
    ...
    def _forward(self, inp):
        if self.pre_lnorm:
            ...
            inp = self.layer_norm(inp)  # [B, N, 384]
            
            core_out = inp.transpose(1, 2)
            core_out = self.CoreNet(core_out.to(inp.dtype))
...
class EEGTransformer(nn.Module):
    def __init__(self, n_layer, n_head, d_model, d_head, d_inner, kernel_size,
                 dropout, dropatt, dropemb=0.0, pre_lnorm=True):
    ...
    def forward(self, dec_inp, padding_mask=None, seq_lens=None, conditioning=0):
        inp = dec_inp
