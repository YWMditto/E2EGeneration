...
from helper_fns import align_length_with_directly_insert, generate_double_casual_mask_batches
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            if casual_mask is not None:
                if casual_mask.size() != (bsz, 1, tgt_len, src_len):
                    raise ValueError(
                        f"Casual mask should be of size {(bsz, 1, tgt_len, src_len)}, but is {casual_mask.size()}"
                    )
...
class HubertEncoderLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, casual_mask=None, output_attentions=False):
        attn_residual = hidden_states
        hidden_states, attn_weights, _ = self.attention(
            hidden_states, attention_mask=attention_mask, casual_mask=casual_mask, output_attentions=output_attentions
        )
        hidden_states = self.dropout(hidden_states)
...
class HubertEncoder(nn.Module):
    def __init__(self, config, specialized_config: "SpecializedPretrainedModelConfig"):
        super().__init__()
        self.config = config
        ...
        if specialized_config.add_phn_embedding:
            self.phn_embedding = nn.Embedding(
                num_embeddings=specialized_config.phn_num,
                embedding_dim=config.hidden_size,
                padding_idx=specialized_config.phn_padding_idx
            )

        # emotion embedding
        if specialized_config.add_emotion_embedding:
            self.emotion_embedding = nn.Embedding(
                num_embeddings=specialized_config.emotion_num,
                embedding_dim=config.hidden_size,
                padding_idx=0
            )
    ...
    def forward(
        ...
        repeated_position_embedding=None,  # 用于在扩增 hidden states 在时间序列上的长度时使用；
        phn_dict=None,
        emotion_ids=None
    ):
        
        double_casual_mask_list = None
        if self.specialized_config.add_double_casual_mask:
            double_casual_mask_list = []
            double_casual_mask = None
            for i in range(len(self.layers)):
                recompute = False
                
                if self.specialized_config.add_phn_embedding:
                    if i + 1 == self.specialized_config.phn_layer:
                        audio_feature_lengths = label_lengths  # 这里是为了在生成 double casual mask 时直接使用匹配的 hidden states 的长度；
                        recompute = True

                if double_casual_mask is None or self.specialized_config.dcm_ratios[i] != self.specialized_config.dcm_ratios[i -1]:
                    recompute = True

                if recompute:
                    double_casual_mask = generate_double_casual_mask_batches(audio_feature_lengths, self.specialized_config.dcm_ratios[i-1])
                    double_casual_mask = 1.0 - double_casual_mask[:, None, :, :].to(dtype=hidden_states.dtype)
                    double_casual_mask = double_casual_mask * torch.finfo(hidden_states.dtype).min
                    double_casual_mask = double_casual_mask.to(hidden_states.device)

                double_casual_mask_list.append(double_casual_mask)
        ...
        for layer in self.layers:
            ...
            # 即使将 phn embedding 和 emotion embedding 加在同一层，也应该先添加 phn embedding，因为涉及到扩充 hidden states 长度的问题；
            # 注意这里 phn 的长度我们已经在提前的数据预处理中将其长度对齐到控制器标签的长度；
            if self.specialized_config.add_phn_embedding and self.specialized_config.phn_layer == layer_idx:
                assert phn_dict is not None
                collated_phns = phn_dict["collated_phns"]
                phn_embeddings = self.phn_embedding(collated_phns)  # [B, N, H]

                phn_lengths = phn_dict["phn_lengths"]  # 这个和 label_lengths 等价；
                max_length = int(max(phn_lengths).item())

                real_hidden_states = []
                for i in range(len(audio_feature_lengths)):
                    cur_feature_length = audio_feature_lengths[i]
                    cur_hidden_state = hidden_states[i]
                    cur_hidden_state = cur_hidden_state[:cur_feature_length]

                    label_length = int(phn_lengths[i].item())
                    if cur_feature_length > label_length:
                        assert False
                        cur_hidden_state = cur_hidden_state[:label_length]
                    elif cur_feature_length < label_length:
                        cur_hidden_state = align_length_with_directly_insert(cur_hidden_state, label_length, repeated_position_embedding)

                    if label_length < max_length:
                        cur_hidden_state = torch.cat([cur_hidden_state, cur_hidden_state.new_zeros(size=(max_length - label_length, cur_hidden_state.size(1)))])                    
                    real_hidden_states.append(cur_hidden_state)
                
                real_hidden_states = torch.stack(real_hidden_states)

                hidden_states = real_hidden_states + phn_embeddings

                # change attention mask
                attention_mask = torch.zeros(
                    (len(hidden_states), max_length), dtype=attention_mask.dtype, device=attention_mask.device
                )
                # these two operations makes sure that all values before the output lengths idxs are attended to

                attention_mask[(torch.arange(attention_mask.shape[0], device=attention_mask.device), phn_lengths - 1)] = 1
            ...
            if self.specialized_config.add_emotion_embedding and self.specialized_config.emotion_layer == layer_idx:
                assert emotion_ids is not None
                emotion_embeddings = self.emotion_embedding(emotion_ids)  # [B, H]
                emotion_embeddings = emotion_embeddings.unsqueeze(1).repeat(1, hidden_states.size(1), 1)
                hidden_states += emotion_embeddings
            # 

            # 添加特殊的 casual mask；
            if self.specialized_config.add_double_casual_mask:
                double_casual_mask = double_casual_mask_list[layer_idx-1]
            ...
            if not skip_the_layer or deepspeed_zero3_is_enabled:
                ...
                if self.gradient_checkpointing and self.training:
                    ...
                    layer_outputs = layer(
                        hidden_states, attention_mask=attention_mask, casual_mask=double_casual_mask, output_attentions=output_attentions
                    )
...
class HubertEncoderStableLayerNorm(nn.Module):
    ...
    def forward(
        ...
        label_lengths=None,
        audio_feature_lengths=None,
        repeated_position_embedding=None,
        phn_dict=None,
        emotion_ids=None  # 注意 model_1 中我们设置的名字是叫 emotion_indices；
    ):
        all_hidden_states = () if output_hidden_states else None
        all_self_attentions = () if output_attentions else None
        ...
        for layer in self.layers:
            ...
            if self.specialized_config.add_phn_embedding and self.specialized_config.phn_layer == layer_idx:
                ...
                phn_lengths = phn_dict["phn_lengths"]
                max_length = int(max(phn_lengths).item())
...
class HubertModel(HubertPreTrainedModel):
    def __init__(self, config: HubertConfig, specialized_config: "SpecializedPretrainedModelConfig"):
        super().__init__(config)
        self.config = config
        ...
        if config.do_stable_layer_norm:
            self.encoder = HubertEncoderStableLayerNorm(config, specialized_config)
        else:
            self.encoder = HubertEncoder(config, specialized_config)
    ...
    def forward(
        ...
        encoder_outputs = self.encoder(
            ...
            repeated_position_embedding=repeated_position_embedding,
            phn_dict=phn_dict,
